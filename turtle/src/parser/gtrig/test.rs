...
use crate::{
    _term::IndexedTerm,
    parser::nq::NQuadsParser,
    serializer::trig::{TriGConfig, TriGSerializer},
    test::{LazyMap, gtrig_samples},
};
...
use sophia_api::{
    ...
    source::{QuadSource, StreamError::SourceError},
...
#[test_case("[] a 42 []", (0, 8); "spurious term outside graph")]
#[test_case("{ [] a 42 [] }", (0, 10); "spurious term inside graph")]
fn err_expected(input: &str, exp_pos: (usize, usize)) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        let p = GTriGParser::new();
        let mut qs = p.parse_str(i);
        let Err(SourceError(err)) = qs.try_for_each_quad(no_callback) else {
            panic!()
        };
        assert!(
...
fn err_invalid_prefix(input: &str, exp_pos: (usize, usize)) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        let p = GTriGParser::new();
        let mut qs = p.parse_str(i);
        let Err(SourceError(err)) = dbg!(qs.try_for_each_quad(no_callback)) else {
            panic!()
        };
        assert!(
...
fn base(input: &str, exp: &str) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        let p = GTriGParser::new();
        let mut qs = p.parse_str(i);
        qs.try_for_each_quad(no_callback)?;
        assert!(qs.extra.base.is_some());
        assert_eq!(qs.extra.base.as_ref().unwrap().as_str(), exp);
    }
    Ok(())
}
...
fn prefix(input: &str, exp1: &str, exp2: &str) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        ...
        let mut qs = p.parse_str(i);
        qs.try_for_each_quad(no_callback)?;
        assert_eq!(qs.extra.prefixes.len(), 1);
        let (got1, got2) = &qs.extra.prefixes[0];
...
fn version(version: Version, keyword: &str, sep: &str, quote: &str, suffix: &str) -> TestResult {
    ...
    let p = GTriGParser::new();
    let mut qs = p.parse_str(&line);
    dbg!(qs.try_for_each_quad(no_callback)).unwrap();
    assert_eq!(qs.inner.version, version);
    Ok(())
}
...
fn unrecognized_version(keyword: &str, sep: &str, quote: &str, suffix: &str) -> TestResult {
    ...
    let mut qs = p.parse_str(&line);
    dbg!(qs.try_for_each_quad(no_callback)).unwrap();
    assert_eq!(qs.inner.version, Version::default());
    Ok(())
}
...
fn iri(input: &str, exp: &str) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        let mut qs = GTriGSource::new(());
        qs.parse_line(i, no_callback)?;
        assert_eq!(&qs.inner.terms[..], &[IndexedTerm::Iri(0)]);
        assert_eq!(qs.inner.buffers.top(), exp);
    }
    Ok(())
}
...
fn bnode(input: &str, exp: &str) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        let mut qs = GTriGSource::new(());
        qs.parse_line(i, no_callback)?;
        assert_eq!(&qs.inner.terms[..], &[IndexedTerm::BlankNode(0)]);
        assert_eq!(qs.inner.buffers.top(), exp);
    }
    Ok(())
}
...
fn boolean_literal(input: &str, exp: bool) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        let mut qs = GTriGSource::new(());
        qs.parse_line(i, no_callback)?;
        assert_eq!(qs.inner.terms.last(), Some(&IndexedTerm::BoolLiteral(exp)));
    }
    Ok(())
}
...
fn numeric_literal(input: &str, exp_lex: &str, exp_dt: &str) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        ...
        let Some(IndexedTerm::XsdLiteral(_, got_dt)) = qs.inner.terms.last() else {
            unreachable!()
        };
        assert_eq!(qs.inner.buffers.top(), exp_lex);
...
fn rdf_literal_lex_multiline() -> TestResult {
    let data = "[] a '''\n a \n '''";
    let mut qs = GTriGSource::new(());
    qs.parse_line(data, no_callback)?;
    assert_eq!(qs.inner.buffers.top(), "\n a \n ");
    Ok(())
}
...
fn rdf_literal_simple(input: &str, exp_lex: &str) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        let mut qs = GTriGSource::new(());
        qs.parse_line(i, no_callback)?;
        let idx = qs.inner.terms.len() - 4; // because a reifying triple was pushed on top
        assert!(matches!(
            dbg!(qs.inner.terms.get(idx)),
            Some(&IndexedTerm::XsdLiteral(_, _))
        ));
        let Some(IndexedTerm::XsdLiteral(idx, got_dt)) = qs.inner.terms.get(idx) else {
            unreachable!()
        };
        assert_eq!(qs.inner.buffers.get(*idx).unwrap(), exp_lex);
...
fn rdf_literal_typed(input: &str, exp_lex: &str, exp_dt: &str) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        ...
        qs.parse_line(i, no_callback)?;
        assert!(matches!(
            dbg!(qs.inner.terms.last()),
            Some(&IndexedTerm::TypedLiteral(_, _))
        ));
        let Some(IndexedTerm::TypedLiteral(idx_lex, idx_dt)) = qs.inner.terms.last() else {
            unreachable!()
        };
        assert_eq!(qs.inner.buffers.get(*idx_lex).unwrap(), exp_lex);
        assert_eq!(qs.inner.buffers.get(*idx_dt).unwrap(), exp_dt);
    }
    Ok(())
}
...
fn rdf_literal_langstr(
    ...
    for i in [input, &format!("{input}\n # this is a comment")] {
        ...
        let Some(IndexedTerm::LangString(idx_lex, idx_tag, got_dir)) = qs.inner.terms.last() else {
            unreachable!()
        };
        assert_eq!(qs.inner.buffers.get(*idx_lex).unwrap(), exp_lex);
        assert_eq!(qs.inner.buffers.get(*idx_tag).unwrap(), exp_tag);
...
fn triple_term(s: &str, p: &str, o: &str) -> TestResult {
    ...
    assert_eq!(g2.len(), 1);
    let [s1, p1, o1] = &g1[0].0;
    let [s2, p2, o2] = g2[0].0[2].triple().unwrap();
...
fn reified_triple(s: &str, p: &str, o: &str) -> TestResult {
    ...
    assert_eq!(d2.len(), 1);
    let [s1, p1, o1] = &d1[0].0;
    let [s2, p2, o2] = d2[0].0[2].triple().unwrap();
    assert_eq!([s1, p1, o1], [s2, p2, o2]);
    Ok(())
}
...
fn count_triples(input: &str, exp: usize) -> TestResult {
    for i in [input, &format!("{input}\n # this is a comment")] {
        ...
        let mut got = 0;
        let mut qs = GTriGSource::<()>::new(());
        qs.parse_line(&data, |_| -> Result<(), Infallible> {
            got += 1;
            println!(">> emitted {got}");
            Ok(())
        })?;
...
static TESTS: LazyMap = gtrig_samples();
...
#[test_case("all bnodes")]
#[test_case("all anon")]
#[test_case("all strings")]
#[test_case("all numbers")]
#[test_case("all variables")]
#[test_case("all triple terms")]
#[test_case("almost all triple terms")]
#[test_case("almost all reified triples")]
#[test_case("almost all bnode property lists")]
#[test_case("almost all collections")]
fn samples(name: &str) {
    let (line, count) = TESTS.get(name).unwrap();
    let mut c = 0;
    let nt_parser = GTriGParser::new();
    nt_parser
        .parse_str(line)
        .for_each_quad(|q| {
            println!("{q:?}");
...
<x:s7> <x:p7> <x:o7> <x:g5> .
    "#;
    let d1: Vec<Spog<SimpleTerm>> = GTriGParser::new().parse_str(TRIG).collect_quads()?;
    let d2: Vec<Spog<SimpleTerm>> = NQuadsParser::new().parse_str(NQ).collect_quads()?;
    assert!(sophia_isomorphism::isomorphic_datasets(&d1, &d2)?);
    Ok(())
}
...
#[test_case("all numbers")]
#[test_case("all variables")]
// #[test_case("all triple terms")] // not supported by isomorphic_datasets
#[test_case("almost all triple terms")]
#[test_case("almost all reified triples")]
#[test_case("almost all bnode property lists")]
#[test_case("almost all collections")]
fn roundtrip_not_pretty(key: &str) -> TestResult {
    let (ttl, _) = TESTS.get(key).unwrap();
    let d1: Vec<Spog<SimpleTerm>> = parse_str(ttl).collect_quads()?;
    ...
    let d2: Vec<Spog<SimpleTerm>> = parse_str(&out).collect_quads()?;
...
#[test_case("almost all bnode property lists")]
#[test_case("almost all collections")]
fn roundtrip_pretty(key: &str) -> TestResult {
    let (ttl, _) = TESTS.get(key).unwrap();
    let d1: Vec<Spog<SimpleTerm>> = parse_str(ttl).collect_quads()?;
    ...
    let d2: Vec<Spog<SimpleTerm>> = parse_str(&pretty).collect_quads()?;
    let ugly = TriGSerializer::new_stringifier()
...
fn no_callback(_: Spog<StashedTerm<'_>>) -> Result<(), Infallible> {
    Ok(())
}
